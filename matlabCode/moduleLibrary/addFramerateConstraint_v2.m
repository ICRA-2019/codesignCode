function [Aineq, bineq] = addFramerateConstraint_v2(Aineq, bineq, modules, maxPxDisplacementFrames, meanGroundDistance, fracMaxSpeed)
...
a_camera =  -4*log( modules.cameras(fIds.framerate,:) ) ...
    - 2 * log( modules.cameras(fIds.weight,:) * 1e-3 * g ) / nrModules;  
a_computer = - 2 * log( modules.computerVIOs(fIds.weight,:) * 1e-3 * g ) / nrModules;
